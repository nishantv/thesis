...
API_KEY = '2737a117ec0e82cc863c7cfaa4d885e4'
API_SECRET = 'ea7c757525cd54d6'
email = None
password = None
AUTH = False
...
class Photo(object):
    ...
    def getComments(self) :
        ...
        try:
            data = _doget(method,  photo_id=self.id)
            comm.append(data.rsp.comments.comment)
...
#XXX: Added Code
def people_findByID(user_id):
    """Returns User object."""
    method = 'flickr.people.getInfo'
    try:
     data = _doget(method,  user_id=user_id)
     person1 =data.rsp.person
    except FlickrError: # ???? what errors might there be????
            return "Unknown error"
    return person1
...
#Returns a list of recently uploaded images. Developed by Aymen
def people_getRecentPhoto(per_page='', page=''):

    """Returns list of Photo objects."""
    method = 'flickr.photos.getRecent'
    data = _doget(method, per_page=per_page, page=page)
...
def people_getPublicGroups(user_id):
    """Gets the groups (Users) for the user_id by Aymen"""
    ...
    try:
     if isinstance(data.rsp.groups.group, list):
            for group in data.rsp.groups.group:
                groups.append(_parse_group(group))
     else:
            groups = [_parse_group(data.rsp.groups.group)]
     return groups
    except AttributeError:
      return "No users in the list"
...
def url_getUserPhotosURL(user_id):
